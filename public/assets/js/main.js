...
function open_popup(pop_id, kol,kol2, kol_text, price, placeholder = '', price2 = 0, price2_active = 0)
{
    popUp.openWindow('#'+pop_id); 
    $('#'+pop_id+' #kol_text').html(kol_text);
    $('#'+pop_id+' #kol').val(kol);
    $('#'+pop_id+' #kol2').val(kol2);
    if (price > 0)
...
setInterval(function () {
    $.post("get_last_order.php", {last_id: last_order_id}, function (data){
        res = $.parseJSON(data);
        if (res.info)
        {
            last_order_id = res.last_id;
            $('#inform_image').html(res.image);
            $('#inform_info').html(res.info);
            if (res.country)
                $('#inform_text').html('Someone in '+res.country+' purchased');
            else
                $('#inform_text').html('Someone purchased');
            informer.init();
        }
    });
}, 10000);
