...
<div class="container">
    <div class="row">
        ...
        <div class="col-md-3 threads shadow m-1 p-2">
            @include('messenger::partials.threads')
      
        </div>

        <div class="col-md-6 shadow m-1 p-2" >
            <div class="panel panel-default">
            <h4 style="text-transform:capitalize;" > 
                <i class="fa fa-user" ></i>&nbsp; Chat With {{$withUser->name}}</h4>
            <hr>
                ...
                <div class="panel-body">
                    <div class="messenger" >
                        ...
                        <div class="messenger-body">
                            @if($withUser->id == 1 && Auth::User()->isAdmin == 1)

                            <h4>Select form threads</h4>
                ...
                <div class="panel-footer">
                    ...
                    <button type="submit" id="send-btn" class="btn btn-primary px-4" style="border-radius:24px;">SEND</button>
                    @endif
                </div>
            </div>
        </div>
...
@section('js-scripts')
    ...
    <script type="text/javascript">
        var withId        = @if(Auth::User()->isAdmin == 1) {{$withUser->id}} @else {{ 1 }} @endif,
            authId        = {{auth()->id()}},
    ...
    <script>
        /**
         src="@{{asset('vendor/messenger/js/messenger-chat.js')}}" charset="utf-8"
...
(function () {
    ...
    function makeSeen(authId, withId) {
        $.ajax({
            url: '{{url('/messenger/ajax/make-seen')}}',
            method: 'POST',
     ...
     */
         ...
         function newMessage(message, messageClass, failed = 0) {
           $('.messenger-body').append('\
                <div class="row message-row"  style="margin:5px; border-radius:24px;">\
                    <p style="border-radius:20px;" class="' + messageClass + '">'
                     +`<i class="fa fa-arrow-circle-right"></i>  &nbsp;` +message.message + '</p>\
    ...
    function loadMessages() {
        $.ajax({
            url: '{{url('/messenger/more/messages')}}',
            method: 'GET',
            data: {
                withId: withId,
                take: take
     ...
     */
    function playTweet() {
        var audio = new Audio('{{asset('vendor/messenger/sounds/tweet.mp3')}}');
        audio.play();
    }
